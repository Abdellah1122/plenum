        ...
        <section className="max-w-[1800px] mx-auto px-6 sm:px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map((item) => (
              <div key={item} className="group cursor-pointer border-b border-border pb-8 hover:border-secondary transition-colors">
                <div className="aspect-[4/3] bg-muted mb-6 relative overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur px-4 py-2 text-xs font-bold tracking-widest text-primary">
                    LOT {item < 10 ? `00${item}` : `0${item}`}
                  </div>
                </div>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-2xl font-heading mb-1">Forme Abstraite {item}</h3>
                    <p className="text-xs font-bold tracking-widest text-secondary uppercase">NOM DE L'ARTISTE</p>
                  </div>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <p className="text-muted-foreground">Est. 5 000 € - 8 000 €</p>
